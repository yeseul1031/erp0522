# Extracted (Table/Domain-specific content)
본 파일은 기존 schema-policy-and-naming-tmp.md에서 **스키마 정책/네이밍(메타)** 범위를 넘어서는
도메인/테이블별 설명·코드북·운영 시나리오 등을 **원문 손상 없이 분리 보관**한 것이다.
