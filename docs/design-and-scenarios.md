...
### 시나리오: 국내 + 해외 혼합 수급 (단일 주문항목)

본 시나리오는 하나의 주문항목(`order_line`)에 대해  
일부 수량은 국내 수급, 일부 수량은 해외 수급으로 나누어 처리하는 경우를 설명한다.  
수급 방식이 다르므로 `sourcing_cases`는 국내/해외 각각 생성되며,  
각 케이스는 독립적인 수급 프로세스를 가진다.
...
**가정**
- 하나의 주문(`orders`)이 이미 생성되어 있다.
- 본 시나리오에서는 주문항목 1건을 기준으로 설명한다.
...
주문항목은 고객이 요구한 총 납품 수량을 의미한다.  
이후 국내/해외 수급으로 분기되더라도,  
최종적으로는 이 주문항목의 요구 수량을 충족해야 한다.
...
- 국내 수급: 60
- 해외 수급: 40
- 총 수량: 100 (주문항목 수량과 동일)

이 분할은 실행 단계의 판단이며,  
주문항목 자체를 분리하지 않고 수급 케이스에서 분기한다.
...
- `sc_sn = 201`
- `ol_sn = 20`
- `sourcing_type = 'DOMESTIC'`
- `status = 'ACTIVE'`

이 레코드는 주문항목 중 **국내로 수급할 부분**에 대한  
수급 전략 및 관리 단위를 정의한다.
...
- `sc_sn = 202`
- `ol_sn = 20`
- `sourcing_type = 'OVERSEAS'`
- `status = 'ACTIVE'`

해외 수급은 국내 수급과 프로세스가 다르므로  
별도의 수급 케이스로 분리하여 관리한다.
...
국내 수급에 필요한 상세 정보는 `domestic_cases`에 기록된다.  
이 레코드는 국내 수급 케이스와 1:1 관계를 가진다.
...
해외 수급에 필요한 상세 정보(직수입 여부 등)는  
`overseas_cases`에 기록되며, 해외 수급 케이스와 1:1 관계를 가진다.
...
해외 수급에 해당하는 수량 40은  
해외 공급사를 통해 별도의 발주 및 수입 프로세스로 진행된다.
...
해외 수급의 경우 선적, 통관, 입고 등  
단계별 진행 상태를 `overseas_steps`로 관리할 수 있다.

---

### 9. 비용 및 귀속 개요

- 국내 발주에서 발생한 매입비 및 부대비용은 `costs`로 기록된다.
- 해외 발주에서 발생한 매입비, 운임, 관세 등도 `costs`로 기록된다.
- 각 비용은 `cost_allocations`를 통해
  - 국내 비용 → `sc_sn = 201`
  - 해외 비용 → `sc_sn = 202`
  로 귀속된다.

이를 통해 동일한 주문항목에 대해  
국내/해외 수급 비용을 명확히 분리하여 관리할 수 있다.

---

### 10. 납품 및 수량 충족

- 국내 및 해외 수급에서 발생한 실물 입고는 각각 관리된다.
- 납품 단계에서는 두 수급 케이스에서 확보된 수량이 합산되어
  주문항목(`ol_sn = 20`)의 요구 수량 100을 충족한다.

---

### 요약

- 하나의 주문항목에 대해 국내/해외 수급이 혼합될 수 있다.
- 이 경우 수급 방식별로 `sourcing_cases`를 분리 생성한다.
- 각 수급 케이스는 독립적인 발주 및 비용 흐름을 가진다.
- 최종적으로는 모든 수급 결과가 하나의 주문항목 요구 수량을 충족한다.
