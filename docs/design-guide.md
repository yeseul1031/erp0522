...
### 4.1 RFQ는 업체별 견적 요청과 회신을 함께 담는다

현재 DDL 기준 견적 흐름에는 별도 `rfq_plans` 계층이 없다. 견적 요청은 `rfqs`와 `rfq_lines`에 바로 생성한다.

- `rfqs`: 특정 업체에 보낸 견적 요청 컨테이너
- `rfq_lines`: 업체에 요청한 라인 정보와 업체 회신 정보를 함께 담는 라인
- `rfq_allocations`: RFQ 라인이 어떤 수급 실행 라인에 귀속되는지 설명

같은 요구를 여러 업체에 비교 견적하려면 업체별로 `rfqs`를 각각 만들고, 각 RFQ 아래에 `rfq_lines`와 `rfq_allocations`를 둔다. 선택 결과는 별도 RFQ 기안 상태가 아니라, 이후 생성되는 `purchase_orders.po_source_rfq_sn`이 어떤 RFQ를 근거로 삼는지로 표현한다.
...
견적과 발주는 단순 1:1만을 전제로 설계하지 않는다. 하나의 계약 요구가 여러 수급 라인으로 나뉠 수 있고, 여러 수급 라인이 하나의 RFQ나 발주에 함께 담길 수도 있다. 반대로 하나의 수급 라인이 여러 견적 비교나 분할 발주로 이어질 수도 있다.

그래서 RFQ와 PO는 SCL을 직접 기준으로 삼지 않고, `rfq_allocations`와 `po_allocations`를 통해 실행 귀속을 표현한다. 이 allocation 구조는 예외 처리를 위한 보조 장치가 아니라, 협업과 분할 수급을 정식으로 수용하기 위한 핵심 설계다.
