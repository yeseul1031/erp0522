        ...
        function renderCeoView() {
          ...
          listPane.innerHTML = PO_APPROVAL_DATA.map(po => {
            ...
            const statusText = isPending ? '결재요청' : (po.approvalStep === 'approved' ? '승인완료' : '검토대기');
        ...
        function init() {
          ...
          renderCompareItem();
          renderCart();
          // renderPoItems(); (removed as it's undefined and handled by updatePoItemsTbody later)
          renderPoList();
  ...
  <div style="font-family:'Malgun Gothic','Apple SD Gothic Neo',sans-serif;max-width:900px;margin:0 auto;background:#fff;">
          ...
          newRows.forEach(po => {
            if (!PO_APPROVAL_DATA.find(p => p.id === po.poNo)) {
              PO_APPROVAL_DATA.unshift({
                ...
                approvalStep: 'pending_ceo', mgrComment: '', ceoComment: '',
                _cartItems: po.items
              });
            }
          });
        ...
        function loadPoDetail(vname, items, status) {
          ...
          if (ceoSimActionsEl) {
            if (state.poStatus === '결재중' || state.poStatus === '승인요청중') {
              ceoSimActionsEl.style.setProperty('display', 'flex', 'important');
            } else {
    ...
    `;
        ...
        const PO_APPROVAL_DATA = [
          {
            ...
            approvalStep: 'pending_ceo',  // pending_mgr | pending_ceo | approved | rejected
        ...
        function setPoListRole(role) {
          poListRole = role;
          showPage('pg-po-list');
          // 대표이사는 기본적으로 전체발주관리가 보이도록 (본인 발주서가 없으므로)
          poListTab = role === 'ceo' ? 'all' : 'mine';
          switchPoTab(poListTab, true);
          ...
          const banners = {
            ...
            ceo: { text: '승인 대기 발주서는 <strong>리스트 항목을 클릭</strong>하여 발주 내용을 확인한 후 최종 승인 또는 반려합니다.', bg: '#fff1f2', border: '#be123c', desc: '대표이사 · 전체 발주 최종 결재' },
          };
        ...
        function renderPoListFull() {
          ...
          let filtered = PO_APPROVAL_DATA.filter(po => {
            if (poListRole === 'buyer') {
              ...
              return po.role === 'mgr' || po.role === 'buyer'; // 전체: 팀원+본인
            } else {
              // CEO: all만 (전체 표시)
              return true;
            }
          ...
          // 중간관리자 전체보기 시 본인 기안을 최상단으로 정렬
          if (poListRole === 'mgr' && poListTab === 'all') {
            filtered.sort((a, b) => {
              if (a.role === 'mgr' && b.role !== 'mgr') return -1;
              if (a.role !== 'mgr' && b.role === 'mgr') return 1;
              return 0;
            });
          }
          ...
          if (filtered.length === 0) {
            tbody.innerHTML = `<tr><td colspan="7" style="text-align:center;padding:32px;color:#94a3b8;">조건에 맞는 발주서가 없습니다.</td></tr>`;
            return;
          }
          ...
          tbody.innerHTML = filtered.map((po, idx) => {
            ...
            return `
              <tr style="${isHighlight ? 'background:#fff8f8;border-left:3px solid #be123c;' : ''}cursor:pointer;" onclick="if(event.target.tagName !== 'BUTTON'){ poListRole==='ceo' ? openCeoReview('${po.id}') : openPoView('${po.id}'); }">
        ...
        function requestApproval() {
          toast('📝 결재 요청이 대표이사에게 상신되었습니다.');
          addLog(`발주서 PO-2026-0156 결재 상신 (결제수단: ${state.poPaymentMethod}) → 대표이사 최종 검토 대기`);
          const mainPo = PO_APPROVAL_DATA.find(p => p.isMain);
          if (mainPo) mainPo.approvalStep = 'pending_ceo';
  ...
  `;
        ...
        function approveOrder() {
          ...
          document.getElementById('approval-result').style.setProperty('display', 'flex', 'important');
    ...
    <table style="width:100%;border-collapse:collapse;font-size:13px;">
      ...
      `).join('')}</tbody>
        ...
        function showPage(id) {
          if (id === 'pg-po-list' && typeof poListRole !== 'undefined' && poListRole === 'ceo') {
            id = 'pg-ceo-approval';
          }
          state.activePage = id;
          ...
          if (id === 'pg-ceo-approval') {
            const el = document.getElementById('pg-ceo-approval');
            el.style.setProperty('display', 'flex', 'important');
            el.style.width = '100%';
            if (typeof renderCeoView === 'function') renderCeoView();
          }
        ...
        function switchProjectTab(tab) {
          ...
          if (tab === 'item') {
            ...
            if (btnArea) btnArea.style.setProperty('display', 'flex', 'important');
            if (tabs[0]) tabs[0].classList.add('active');
          } else if (tab === 'price') {
            if (tableArea) tableArea.style.display = 'none';
            if (priceArea) priceArea.style.setProperty('display', 'flex', 'important');
        ...
        let replyFilterVendor = '';
        let replyFilterStatus = '';
        ...
        function renderReplyTable() {
          const body = document.getElementById('reply-table-body');
          if (!body) return;
          let vids = Object.keys(VNAMES);

          // Apply Filter: Vendor Name
          if (replyFilterVendor) {
            const query = replyFilterVendor.toLowerCase();
            vids = vids.filter(vid => VNAMES[vid].toLowerCase().includes(query));
          }

          // Compute mapped statuses for all replies
          const mappedStatuses = {};
          vids.forEach(vid => {
            const reply = VENDOR_REPLIES[vid] || { status: '입력대기', subtotal: 0 };
            let st = reply.status;
            if (st === '회신완료' || st === '입력완료') st = '회신완료';
            else if (st === '기한만료' || st === '기한_만료') st = '기한만료';
            else st = '입력대기';
            mappedStatuses[vid] = st;
          });

          // Apply Filter: Status
          if (replyFilterStatus) {
            vids = vids.filter(vid => mappedStatuses[vid] === replyFilterStatus);
          }

          // Pagination
          const totalItems = vids.length;
          const totalPages = Math.ceil(totalItems / replyPageSize) || 1;
          if (replyCurrentPage > totalPages) replyCurrentPage = totalPages;

          const startIdx = (replyCurrentPage - 1) * replyPageSize;
          const pagedVids = vids.slice(startIdx, startIdx + replyPageSize);

          body.innerHTML = pagedVids.map(vid => {
            const vname = VNAMES[vid];
            let displayStatus = mappedStatuses[vid];
            let statusBadgeClass = 'badge-amber';
            if (displayStatus === '회신완료') statusBadgeClass = 'badge-green';
            else if (displayStatus === '기한만료') statusBadgeClass = 'badge-red';
            else statusBadgeClass = 'badge-gray';
            ...
            if (state.vendorStatuses[vid] === 'blocked') {
              ...
              displayStatus = '직접입력'; // keep existing UI logic for blocked
              linkHtml = `<span class="link-blocked">🔒 링크차단 ✕</span>`;
            }

            const projectName = "보령-농협 프로젝트";

            return `
          <tr style="cursor:pointer;" onclick="if(!['INPUT','BUTTON','A','SELECT','OPTION','TEXTAREA'].includes(event.target.tagName)) openRfqView('${vid}')">
            <td><input type="checkbox" class="reply-check" data-vid="${vid}" data-status="${displayStatus}" onchange="updateDeleteBtn()"></td>
            <td><div style="font-size:11px;color:#64748b;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">${projectName}</div></td>
        ...
        `;
          ...
          if (paginationContainer) {
            ...
            for (let i = 1; i <= totalPages; i++) {
              pageHtml += `<button type="button" class="btn btn-sm ${i === replyCurrentPage ? 'btn-primary' : 'btn-outline'}" onclick="replyCurrentPage=${i};renderReplyTable()">${i}</button>`;
            }
        ...
        function deleteSelectedReplies() {
          ...
          if (!confirm(`선택한 ${vidsToDelete.length}개의 기한만료 항목을 숨기시겠습니까?`)) return;
      ...
      `).join('');
        ...
        let selectedPoForDetail = null;

        function openRfqView(vid) {
            const vname = VNAMES[vid] || '거래처';
            document.getElementById('rfq-view-vendor-name').textContent = vname;
            const reply = VENDOR_REPLIES[vid];
            if (reply && (reply.status === '회신완료' || reply.status === '입력완료')) {
                document.getElementById('rfq-view-total-amount').textContent = (reply.subtotal * 1.1).toLocaleString();
                document.getElementById('rfq-view-items').innerHTML = `
                  <tr><td style="padding:8px; border-right:1px solid #cbd5e1; border-bottom:1px solid #cbd5e1; text-align:center;">1</td><td style="padding:8px; border-right:1px solid #cbd5e1; border-bottom:1px solid #cbd5e1;">전동기 외 4건</td><td style="padding:8px; border-right:1px solid #cbd5e1; border-bottom:1px solid #cbd5e1; text-align:center;">1 식</td><td style="padding:8px; border-right:1px solid #cbd5e1; border-bottom:1px solid #cbd5e1; text-align:right;">${reply.subtotal.toLocaleString()}</td><td style="padding:8px; border-bottom:1px solid #cbd5e1; text-align:right;">${(reply.subtotal * 1.1).toLocaleString()}</td></tr>
                `;
            } else {
                document.getElementById('rfq-view-total-amount').textContent = '0';
                document.getElementById('rfq-view-items').innerHTML = `
                  <tr><td colspan="5" style="text-align:center; padding:20px;">아직 회신되지 않은 견적입니다.</td></tr>
                `;
            }
            openModal('modal-rfq-view');
        }
        ...
        function openPoView(poId) {
            ...
            document.getElementById('po-view-vendor-name').textContent = po.vendor;
            document.getElementById('po-view-po-no').textContent = po.id;
            document.getElementById('po-view-date').textContent = po.date;
            document.getElementById('po-view-total-amount').textContent = po.total.toLocaleString();
            
            const tbody = document.getElementById('po-view-items');
            if (po._cartItems && po._cartItems.length > 0) {
               tbody.innerHTML = po._cartItems.map((it, i) => `<tr><td style="padding:8px; border-right:1px solid #cbd5e1; border-bottom:1px solid #cbd5e1; text-align:center;">${i+1}</td><td style="padding:8px; border-right:1px solid #cbd5e1; border-bottom:1px solid #cbd5e1;">${it.name}</td><td style="padding:8px; border-right:1px solid #cbd5e1; border-bottom:1px solid #cbd5e1; text-align:center;">${it.qty}${it.unit||'EA'}</td><td style="padding:8px; border-right:1px solid #cbd5e1; border-bottom:1px solid #cbd5e1; text-align:right;">${(it.price||0).toLocaleString()}</td><td style="padding:8px; border-bottom:1px solid #cbd5e1; text-align:right;">${((it.price||0) * (it.qty||1)).toLocaleString()}</td></tr>`).join('');
            } else {
               tbody.innerHTML = `<tr><td style="padding:8px; border-right:1px solid #cbd5e1; border-bottom:1px solid #cbd5e1; text-align:center;">1</td><td style="padding:8px; border-right:1px solid #cbd5e1; border-bottom:1px solid #cbd5e1;">발주 품목 일체</td><td style="padding:8px; border-right:1px solid #cbd5e1; border-bottom:1px solid #cbd5e1; text-align:center;">1 식</td><td style="padding:8px; border-right:1px solid #cbd5e1; border-bottom:1px solid #cbd5e1; text-align:right;">${Math.round(po.total/1.1).toLocaleString()}</td><td style="padding:8px; border-bottom:1px solid #cbd5e1; text-align:right;">${po.total.toLocaleString()}</td></tr>`;
            }

            const editBtn = document.getElementById('po-view-edit-btn');
            if (poListRole === 'buyer' || poListRole === 'mgr') {
               editBtn.style.display = 'inline-block';
            } else {
               editBtn.style.display = 'none';
            }
            
            openModal('modal-po-view');
        }

        function openPoDetailFromView() {
            if(!selectedPoForDetail) return;
            closeModal('modal-po-view');
            const po = selectedPoForDetail;
            loadPoDetail(po.vendor, po._cartItems || [], po.approvalStep === 'approved' ? '승인완료' : po.approvalStep === 'rejected' ? '승인반려' : '결재중');
        }
