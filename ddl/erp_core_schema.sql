 ...
 * 본 도메인의 엔티티들은 모두 공통으로 사용되는 엔티티로서 타 도메인의 엔티티들이 직접 연결 (FK)할 수 있다. (1:N인 경우엔 여전히 _links가 필요하다)
 *
 * ----------------------------------------------------------------------------
 * [DDL 편집 및 유지 원칙 — 중요]
 *
...
CREATE TABLE documents (
  ...
  doc_category VARCHAR(30) NOT NULL COMMENT '문서 출처 테이블명(대문자): PROJECTS, ORDERS, ORDER_LINES, ORDER_LINE_OVERRIDES, COSTS, PAYABLES, PAYMENTS, RFQS, PURCHASE_ORDERS 등',
