...
var Vue = {
    createApp: function(config) {
        // Run data, computed, methods, mounted to see if anything throws
        try {
            console.log("Calling data()...");
            var dataObj = config.data();
            console.log("Data keys:", Object.keys(dataObj));
            
            var context = Object.assign({}, dataObj, config.methods || {});
            
            // Stub compute properties if any
            if (config.computed) {
                for (var key in config.computed) {
                    context[key] = config.computed[key].call(context);
                }
            }
            
            if (config.mounted) {
                console.log("Calling mounted()...");
                config.mounted.call(context);
            }
            console.log("Vue stub execution successful");
        } catch (e) {
            console.log("VUE_INIT_ERROR: " + e.message + "\nStack: " + e.stack);
            throw e;
        }
        return {
            mount: function() {}
