...
var document = {
    getElementById: function() { return { value: '', textContent: '', style: {}, appendChild: function() {}, click: function() {}, addEventListener: function() {} }; },
...
var console = {
    log: function() {}
        ...
        const VENDORS = [
          { id: 'v1', name: '㈜대성상사', ceo: '김대성', email: 'bid@daesung.co.kr', tel: '02-555-1234', addr: '서울시 구로구 디지털로 123' },
          { id: 'v2', name: '㈜한솔무역', ceo: '박한솔', email: 'rfq@hansol-trade.com', tel: '02-888-5678', addr: '서울시 영등포구 경인로 456' },
          { id: 'v3', name: '㈜미래기술', ceo: '이미래', email: 'purchase@miratech.kr', tel: '031-777-9000', addr: '경기도 판교 테크노밸리 789' },
          { id: 'v4', name: '㈜글로벌서플라이', ceo: '최글로', email: 'supply@global-sup.co.kr', tel: '02-111-2222', addr: '서울시 마포구 양화로 12' },
          { id: 'v5', name: '㈜디지털파트너', ceo: '정디지', email: 'bid@dpartner.co.kr', tel: '02-333-4444', addr: '서울시 성동구 성수동 34' },
          { id: 'v6', name: '㈜스마트오피스', ceo: '강스마', email: 'rfq@smartoffice.kr', tel: '02-555-6666', addr: '서울시 강남구 테헤란로 56' },
          { id: 'v7', name: '㈜코리아IT', ceo: '윤코리', email: 'sales@korea-it.com', tel: '02-777-8888', addr: '서울시 중구 세종대로 78' },
          { id: 'v8', name: '㈜베스트서플라이', ceo: '임베스', email: 'best@supply.co.kr', tel: '02-999-0000', addr: '서울시 용산구 한강대로 90' },
        ];
        ...
        function downloadRfqExcel() {
          try {
            ...
            const rows = [...body.querySelectorAll('tr')].map(tr => {
              const cells = tr.querySelectorAll('td');
              return {
                '거래처': cells[0]?.textContent?.trim() || '',
                '입력상태': cells[2]?.textContent?.trim() || '',
                '합계(부가세별도)': cells[3]?.textContent?.trim() || '',
                '합계(부가세포함)': cells[4]?.textContent?.trim() || '',
                '발송상태': cells[6]?.textContent?.trim() || ''
              };
            });
  ...
  <div style="font-family:'Malgun Gothic','Apple SD Gothic Neo',sans-serif;max-width:900px;margin:0 auto;background:#fff;">
    ...
    <tr>
      ...
      <td style="font-weight:600;">통신산업</td>
      <td style="color:#2563eb; font-weight:600;">PO-20260203-0003 식판 취급설비 납품 건</td>
    ...
    <tr>
      ...
      <td style="font-weight:600;">유진공업</td>
      <td style="color:#2563eb; font-weight:600;">PO-20260203-0002 경상자재 예비 부품 건</td>
      ...
      <td style="text-align:center;"><button class="btn btn-purple btn-sm" onclick="loadPoDetail('유진공업', [], '승인반려')">상세 →</button></td>
          ...
          const newRowsHtml = newRows.map((po, idx) => `
    <tr style="background:#f0f7ff;">
      <td style="text-align:center;">${newRows.length - idx + 1}</td>
      <td class="text-xs">${po.date}</td>
      <td style="font-weight:600;">${po.vendor}</td>
      <td style="color:#2563eb; font-weight:600;">
        <i class="ri-file-list-3-line"></i> ${po.poNo} ${po.items[0].name} 외 ${po.items.length - 1}건
      </td>
      <td class="font-mono" style="text-align:right; font-weight:700;">${po.total.toLocaleString()}원</td>
      <td><span class="badge badge-amber">작성중</span></td>
      <td style="text-align:center;"><button class="btn btn-purple btn-sm" onclick="loadPoDetail('${po.vendor}', ${JSON.stringify(po.items).replace(/"/g, "'")}, '작성중')">상세 →</button></td>
    </tr>`).join('');

          document.getElementById('po-list-body').innerHTML = newRowsHtml + existingRow;
        }
    ...
    `;
        ...
        function requestApproval() {
          toast('📝 결재 요청이 상신되었습니다. 대표이사 결재 시뮬레이션 패널에서 승인/반려를 해보세요.');
          addLog(`발주서 PO-2026-0156 결재 상신 (결제수단: ${state.poPaymentMethod})`);
    ...
    <table style="width:100%;border-collapse:collapse;font-size:13px;">
      ...
      `).join('')}</tbody>
        ...
        function renderReplyTable() {
          ...
          body.innerHTML = vids.map(vid => {
            ...
            let historyBadge = '';
            if (lastStatus === 'success') historyBadge = `<span class="badge badge-teal" style="cursor:pointer;" onclick="showSendHistory('${vid}')">✅ 발송성공</span>`;
            else if (lastStatus === 'failed') historyBadge = `<span class="badge badge-red" style="cursor:pointer;" onclick="showSendHistory('${vid}')">❌ 발송실패</span>`;
            else historyBadge = '<span class="badge badge-amber">대기중</span>';
          ...
          <tr>
            ...
            <td class="font-mono text-right">${(reply.subtotal || 0).toLocaleString()}원</td>
            <td class="font-mono text-right">${Math.round((reply.subtotal || 0) * 1.1).toLocaleString()}원</td>
            <td>
              <div class="flex gap2">
                <button class="btn btn-outline btn-sm" onclick="openRfqPreview('${vid}')">PDF</button>
                <button class="btn btn-outline btn-sm" style="color:#16a34a;border-color:#86efac;" onclick="downloadVendorExcel('${vid}')">Excel</button>
                <button class="btn btn-outline btn-sm" onclick="openBlockModal('${vid}')">직접입력</button>
              </div>
            </td>
            <td>${historyBadge}</td>
            <td>
              <div class="flex center">
                <button class="btn btn-primary btn-sm" onclick="retrySend('${vid}')">재발송</button>
              </div>
            </td>
          </tr>
        `;
        ...
        function showSendHistory(vid) {
          ...
          if (infoEl) infoEl.textContent = `${vname} — ${history[0]?.vendorEmail || ''}`;
      ...
      `).join('');
        ...
        function retrySend(vid) {
          ...
          document.getElementById('retry-vfax').value = vendor.fax || '';
        ...
        function confirmRetrySend() {
          ...
          const newFax = document.getElementById('retry-vfax').value;
          ...
          setTimeout(() => {
            ...
            const newEntry = {
              ...
              failReason: fileNames ? '첨부파일: ' + fileNames : '재발송 완료 (추가 첨부 없음)'
            };

            SEND_HISTORY.push(newEntry);

            // 거래처 마스터 정보 업데이트 (이메일, 연락처, 팩스)
            const vendor = VENDORS.find(v => v.id === vid);
            if (vendor) {
